...
from oligo_melting_temp             import basictm, tmbresluc
from seq31                          import seq1
from parse_string                   import parse_seqs, guess_alphabet
...
def tm(wiki, evt):
    ...
    wiki.displayMessage("Primer melting temperature",
                        textwrap.dedent(
                        '''
                        Nearest Neighbour StLucia1998: \t{0} °C
                        Bresl 1986 + StLucia 1998 1µM: \t{1} °C
                        (A+T)*2+(G+C)*4: \t{2} °C
                        GC: \t{3}
                        length: \t{4}-mer
